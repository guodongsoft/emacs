...
;; p   print the entire stack constents
;;
...
(defun calc-print-stack ()
  "Print entire contents of statck, from top to bottom."
  (if calc-stack
      (progn
        (insert "\n")
        (let ((stk calc-stack))
...
(defun calc-operate (tok)
  "Given an arithmetic operator (as string), pop two numbers off the stack,
perform operation tok (given as string), push the result onto the stack."
  (let ((op1 (calc-pop))
        (op2 (calc-pop)))
    (calc-push (funcall (read tok) op2 op1))))

(defun calc-push-number (tok)
  "Given a number (as string), push it (as number) onto the stack."
...
and set name of function to use to process the token."
  (let (tok)
    (cond ((looking-at calc-number-regexp)
          ...
          (t
           (setq calc-proc-fun 'calc-invalid-tock)))
    ;; pick up token add advance past it (and past whitespace)
